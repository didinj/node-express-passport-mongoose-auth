...
function addContestToList(title, reg, type, instruct, desc, opts){
	Contest.create(new Contest({title: title, region: reg, contesttype: type, instructions: instruct, description: desc, options: opts}), function(err,contest){
			if(err){
				console.log(err);
			}else{
...
Ballot.create(new Ballot({title: "Voteyum 2017", contests: contestList}), function(err, ballot){
    if(err){
    	console.log(err);
    }else{
    	console.log(ballot);
